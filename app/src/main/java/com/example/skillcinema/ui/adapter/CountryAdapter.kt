...
class CountryAdapter(private val onClick: (CountryFilter) -> Unit) :
    RecyclerView.Adapter<CountryGenreVH>() {

    private var values: List<CountryFilter> = emptyList()
    private var previousElementId = 0
    private var currentElement = 0

    @SuppressLint("NotifyDataSetChanged")
    fun setData(data: List<CountryFilter>, country: Int?) {
        this.values = data
    ...
    override fun onBindViewHolder(holder: CountryGenreVH, position: Int) {
        ...
        holder.binding.textCountryGenre.text = values[position].country
...
class CountryGenreVH(val binding: CountryGenreFilterBinding) : RecyclerView.ViewHolder(binding.root)